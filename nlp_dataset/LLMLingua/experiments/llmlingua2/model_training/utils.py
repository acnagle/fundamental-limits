...
class TokenClfDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.labels is not None:
            ...
            # insert false labels for the query tokens
            labels = [False] * (len(tokenized_query) + 1) + labels      # +1 for the [SEP] token between query and text

            # insert false labels for the special tokens to be added
            labels.insert(0, False)
            labels.insert(-1, False)
        else:
            tokenized_text = self.tokenizer.tokenize(text)

        tokenized_text = [self.cls_token] + tokenized_query + [self.sep_token] + tokenized_text + [self.sep_token]
        assert len(tokenized_text) == len(labels) if self.labels is not None else True

        if len(tokenized_text) > self.max_len:
            #print("Truncating from {} to {}".format(len(tokenized_text), self.max_len))
            #print('####################')
            #print(query)
            #print('--------------------')
            #print(text)
        ...
        #attn_mask[1: len(tokenized_query) + 1] = [0] * len(tokenized_query)  # set attention mask for query tokens to 0
