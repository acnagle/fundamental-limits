...
parser.add_argument("--model_name", help="llm used to compress", default="gpt-4o")
...
parser.add_argument("--prompt_id", type=int, default=5)
parser.add_argument("--n_max_new_token", type=int, default=4096)
...
for sample in tqdm(data):
    ...
    query = copy.deepcopy(sample["query"])
    if origin is None:
        continue
    ...
    if args.compressor == "llmlingua" or args.compressor == "longllmlingua":
        ...
        for j, chunk in enumerate(origin):
            if args.compressor == "gpt4":
                if comp_list:       # add compressed prompt from previous chunk to the beginning of this chunk
                    chunk = comp_list[-1] + "\n\n" + chunk
                chunk_list.append(chunk)
                comp = compressor.compress(chunk, query, args.n_max_new_token)
            elif args.compressor == "sc":
        ...
        assert len(chunk_list) == len(comp_list)
        comp = "".join(comp_list)
        #comp = comp_list[-1]    # NOTE: since the compressed prompts are added to the beginning of the next chunk, the last chunk is the final compressed prompt
    ...
    if (
        ...
        #new_sample["prompt_list"] = chunk_list[:]  # NOTE: save the chunked prompts instead of the original prompts
