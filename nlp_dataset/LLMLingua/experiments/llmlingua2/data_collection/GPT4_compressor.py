...
class PromptCompressor:
    ...
    def query_template(self, text, query, n_max_new_token=4096):
        if self.user_prompt and "{text_to_compress}" in self.user_prompt and "{query}" in self.user_prompt:
            prompt = self.user_prompt.format(query=query, text_to_compress=text)
        else:
            prompt = text
    ...
    def compress(self, text, query, n_max_new_token=4096):
        messages = self.query_template(text, query, n_max_new_token)
        comp = None
        while comp is None:
            try:
                request = {
