...
for sid, sample in raw_data.items():
    if len(sample["prompt_list"]) != len(sample["compressed_prompt_list"]):
        ...
        continue
    query_list = [sample["query"]] * len(sample["prompt_list"])
...
for chunk_idx, (query, origin, comp) in tqdm(enumerate(zip(queries, origins, comps))):
    num_sample += 1
