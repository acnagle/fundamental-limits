...
class TokenClfDataset(Dataset):
    def __init__(
        ...
        elif "xlm-roberta-large" in model_name or "xlm_roberta_large" in model_name:
            self.bos_token = "<s>"
            self.eos_token = "</s>"
            self.sep_token = "</s>"
            self.cls_token = "<s>"
    ...
    def __getitem__(self, index):
        ...
        if self.query:      # if the query is provided, prepend it to the text
            tokenized_query = self.tokenizer.tokenize(self.query)
            tokenized_text = (
                [self.cls_token] + tokenized_query + [self.sep_token] + tokenized_text + [self.sep_token]
            )
            end_of_query_idx = len(tokenized_query) + 2
        else:
        ...
        return {
            ...
            "end_of_query_idx": end_of_query_idx,
        }
