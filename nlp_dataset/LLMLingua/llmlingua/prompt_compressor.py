...
class PromptCompressor:
    ...
    def load_model(
        ...
        if os.path.exists(model_name):
            ...
            tokenizer_path = os.path.join(model_name, "tokenizer")
            print(f"Using the tokenizer from {tokenizer_path}.")
            tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, **model_config)
        else:
    ...
    def compress_prompt(
        ...
        strict_preserve_uncompressed: bool = True,
        adaptive_query_select: bool = False,
    ):
        """
        ...
        if self.use_llmlingua2:
            return self.compress_prompt_llmlingua2(
                ...
                adaptive_query_select=adaptive_query_select,
            )
        assert (
            rate <= 1.0
    ...
    def compress_prompt_llmlingua2(
        ...
        Args:
            context (List[str]): List of context strings that form the basis of the prompt.
            query (str, optional): A specific question that the prompt is addressing. Default is an empty string.
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                for j in range(ids.shape[0]):
                    ...
                    if "xlm-roberta-large" in self.model_name or "xlm_roberta_large" in self.model_name:
    ...
    def __compress(
        ...
        dataset = TokenClfDataset(
            chunk_list, query, tokenizer=self.tokenizer, max_len=self.max_seq_len, model_name=self.model_name
        )
        dataloader = DataLoader(
            dataset, batch_size=self.max_batch_size, shuffle=False, drop_last=False
        )
        ...
        with torch.no_grad():
            for batch in dataloader:
                ids = batch["ids"].to(self.device, dtype=torch.long)
                mask = batch["mask"].to(self.device, dtype=torch.long) == 1
                end_of_query_idx = batch['end_of_query_idx'][0]     # this value will be the same for all chunks, so we can just use the first one
                assert torch.all(batch['end_of_query_idx'] == batch['end_of_query_idx'][0]), "end_of_query_idx should be the same for all chunks in a batch"

                outputs = self.model(input_ids=ids, attention_mask=mask)
                loss, logits = outputs.loss, outputs.logits
                probs = F.softmax(logits, dim=-1).to(torch.float32)
                probs = probs[:, end_of_query_idx:, :]
                ...
                for j in range(ids.shape[0]):
                    ...
                    #chunk_mask = mask[j]
                    chunk_probs = probs[j, :, 1]
                    chunk_ids = ids[j, end_of_query_idx:]
                    chunk_mask = mask[j, end_of_query_idx:]
                    ...
                    if drop_consecutive:
                        if adaptive_query_select:
                            threshold = reduce_rate
                        else:
