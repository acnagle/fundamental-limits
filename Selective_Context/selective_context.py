...
def tokenize_character_level(text, tokenizer, device):
    # Convert text into a list of characters
    chars = list(text)

    # Manually encode characters by converting each character to its input IDs.
    # Here, we map each character to its corresponding Unicode code point as its ID.
    input_ids = [tokenizer.bos_token_id] + [tokenizer.convert_tokens_to_ids(char) for char in chars]

    # Convert input_ids list to a PyTorch tensor
    input_ids_tensor = torch.tensor([input_ids]).to(device)  # Create a batch dimension
...
class SelectiveContext:
    ...
    def __init__(self, model_type = 'gpt2', lang = 'en', load_local = False, force_tokenization = False):
    ...
    def _prepare_model(self):
        if self.model_type == 'gpt2':
            ...
            self.get_self_information = self._get_self_info_via_mistral
        elif 'pythia' in self.model_type.lower():
            self.tokenizer = AutoTokenizer.from_pretrained('EleutherAI/pythia-1b-deduped')
            if self.load_local:
                self.model = AutoModelForCausalLM.from_pretrained(self.model_type)
            else:
                self.model = AutoModelForCausalLM.from_pretrained('EleutherAI/pythia-1b-deduped')
            self.model.to(self.device)
            self.model.eval()

            self.max_token_length = self.model.config.max_position_embeddings
            self.get_self_information = self._get_self_info_via_pythia
    ...
    def _get_self_info_via_mistral(self, text: str) -> Tuple[List[str], List[float]]:
        with torch.no_grad():
            if self.force_tokenization:
                encoding = tokenize_character_level(text, self.tokenizer, self.device)
            else:
    ...
    def _get_self_info_via_pythia(self, text: str) -> Tuple[List[str], List[float]]:
        with torch.no_grad():
            if self.force_tokenization:
                encoding = tokenize_character_level(text, self.tokenizer, self.device)
            else:
